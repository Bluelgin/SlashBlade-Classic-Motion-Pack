...
## A3 / Sakura-right quick-draw boundary

The first PoC used `Battou` in A3 and held its terminal pose until frame 281 because that is where Resharped enters its final A3/Sakura recovery segment. Further source comparison showed that r32 has two distinct exits from `Saya2`: a fast/high-rank route to `SIai`, and a late/low-rank route to `Battou`. Resharped likewise has an early A2 exit to A3 and a later A2-end exit to C. The pack therefore uses `SIai` for A3 and keeps `Battou` on C.

Both r32 `SIai` and `Battou` have a `comboResetTicks` value of 12. This is **not** the visible swing duration: the actual weapon swing still uses the short vanilla-swing-derived curve described above. It is, however, the old combo-state timeout after which a drawn move falls into `Noutou`. At the VMD playback rate used here, 12 game ticks correspond to 18 VMD frames. Starting from frame 200 gives frame **218**.
...
The shared 200–314 atlas region therefore now holds the short classic `SIai` swing, keeps its terminal pose through the remainder of the old 12-tick window, and begins the Noutou-style recovery at frame 218. Sakura End Right necessarily sees the same atlas content because the two moves share resource/frame addresses.
