...
- Referenced files: `item/ItemSlashBlade.java` (ComboSequence and transitions) and `client/renderer/entity/layers/LayerSlashBlade.java` (progress function, transformation order, blade/saya drawing and classic trail rendering), under `src/main/java/mods/flammpfeil/slashblade/`.
- Shipped derivatives: `tools/vmd/legacy.py`, `data/legacy_motion_map.json`, and generated `assets/slashblade/combostate/motion.vmd`. These are offline adaptations of procedural source behavior; they are not copied old binary art assets.
...
The pack does **not** ship r32's old `trail.obj`/`trail.png`. Runtime testing showed that a resource-only simulated classic ribbon remained out of sync with Resharped's Java-owned `EntitySlashEffect` rotation, so the default pack now suppresses the normal slash-light visual instead. The generated `slash.obj`/`slash.png` are original suppression assets: a microscopic valid quad and fully transparent texture. The pack does not ship `drive.obj`, `ss.png`, `slashdim.obj` or `slashdim.png`; those SA effect assets remain supplied by the installed Resharped/addon environment. No legacy texture, model or VMD binary is shipped. The official JAR was identified but its binary could not be downloaded in the original research environment, so JAR-to-source bytecode equivalence is not asserted.
...
- Redistribution status: upstream modern art is **not** included. `slash.obj/png` in this pack are generated project outputs rather than copies of Resharped's art. `drive.obj`, `ss.png` and `slashdim.obj/png` are deliberately not redistributed or overridden. The pack uses the existing installed bladeholder and game blade models. Its `model/pa/alex.pmd` is a new bone-only format adapter, not a modified copy of upstream Alex. Its player VMD contains newly generated neutral adapter keys, not upstream animation.
...
The existing root MIT license applies to original tooling, tests, documentation, metadata, geometric icon and generated slash-suppression mesh/texture. It does not change the terms of upstream references or the legacy-derived procedural motion noted above. No upstream asset is claimed as our original art. This notice is included in the runtime ZIP.