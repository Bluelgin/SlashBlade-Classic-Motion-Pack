...
The Piercing generator is intentionally not based on either upstream modern binary. It rebuilds the modern 1–90 contract from source semantics: r32 `None` before modern activation, full r32 `Stinger` from frame 33, the old 20-tick reset clock converted to frame 63, a fresh Noutou swing, then neutral. The player VMD contains only `classic_root`, matching the generated bone-only PMD adapter. See `docs/research/piercing-classic-interpretation.md`.
